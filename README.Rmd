...
#Delete an answer with incorrect information
question_7$numeric_answer[question_7$numeric_answer=="5,10"]<- NA
question_7$round_num_answer[question_7$round_num_answer=="5,10"]<- NA
question_7$numeric_answer<- as.numeric(question_7$numeric_answer)
question_7$round_num_answer<- as.numeric(question_7$round_num_answer)
#average by department (weighted and no weighted)
#not weighted 
avg_dept_no_weight_7<- question_7 %>%
  group_by(crop, expert_in) %>%
  mutate(numeric_answer=as.numeric(numeric_answer))%>%
  mutate(average= mean(numeric_answer, na.rm = T)) %>%
  group_by(average)%>%
...
avg_dept_weight_7<- add_weights_question_7 %>%
  group_by(crop, expert_in) %>%
  mutate(numeric_answer=as.numeric(numeric_answer))%>%
  mutate(average_weighted= weighted.mean(numeric_answer, weights, na.rm = T)) %>% 
  group_by(average_weighted)%>%
...
nal_q7_no_weight<- question_7 %>% group_by(expert_in) %>%
  summarise(average= mean(as.numeric(numeric_answer), na.rm = T)) %>%
  group_by(average) %>% 
...
nal_q7_weight<- nal_q7_weight %>%
  group_by(expert_in) %>%
  mutate(numeric_answer=as.numeric(numeric_answer)) %>%
  summarise(average_weighted= weighted.mean(numeric_answer, weights, na.rm = T)) %>% 
  group_by(average_weighted) %>%
...
question_12_long_frequencies_weighted_net<- question_12_long_frequencies_weighted %>%
  mutate(crop= case_when(
    crop=="Banana" ~"BN,BC",
    crop=="Plantain" ~ "PN,PC"
  ))
# Separate the "crop" column by commas and create new rows

question_12_long_frequencies_weighted_net <- separate_rows(question_12_long_frequencies_weighted_net, crop, sep = ",") 
colnames(question_12_long_frequencies_weighted_net)<- c("Value", "From", "To",  "freq_unique")
# This data will be used for the weighted edges:
write.csv(question_12_long_frequencies_weighted_net, "./Data/SUP_Proportion_of_movement_of_planting_materials_by_departments.csv", row.names = FALSE)
...
write.csv(rep_question_15_weighted, "./Data/SUP_question_15_weighted.csv")
...
#Selecting the questions regarding to the correct utilization of management practices
heat_map_data_correct<- rbind(avg_dept_weight_16, avg_dept_weight_17, avg_dept_weight_18, avg_dept_weight_20)
#Calculating the mean by crop by expert by question number. Every question number is a different management practice
heat_map_data_matrix_correct<-  heat_map_data_correct %>%
  group_by(crop, expert_in, question_number) %>%
  summarise(value = mean(average_weighted))
#Format the data set to long
heat_map_data_matrix_correct<- dcast(data = heat_map_data_matrix_correct, formula = crop+expert_in~question_number)
#Standardazing the data
mat<- heat_map_data_matrix_correct[, c("16", "17", "18", "20")]
mat<- mat/max(mat)
#Adding a blank space to the banana observations 
#dput(heat_map_data_matrix_correct$expert_in)
rownames(mat)<-c("Antioquia", "Caldas", "Cesar", "Chocó", "Córdoba", "Huila", 
"La Guajira", "Magdalena", "Quindío", "Risaralda", "Antioquia ", 
"Arauca", "Atlántico", "Bolívar", "Caldas ", "Casanare", "Cauca", 
"Córdoba ", "Huila ", "La Guajira ", "Magdalena ", "Meta", "Nariño", 
"Norte De Santander", "Quindío ", "Risaralda ", "Santander", "Sucre", 
"Tolima", "Valle Del Cauca")


#Selecting the questions regarding to the preceived effectiveness of management practices
heat_map_data_perceived<- rbind(avg_dept_weight_22, avg_dept_weight_23, avg_dept_weight_24, avg_dept_weight_25)
#Calculating the mean by crop by expert by question number. Every question number is a different management practice
heat_map_data_matrix_perceived<-  heat_map_data_perceived %>%
  group_by(crop, expert_in, question_number) %>%
  summarise(value = mean(average_weighted))
#Format the data set to long
heat_map_data_matrix_perceived<- dcast(data = heat_map_data_matrix_perceived, formula = crop+expert_in~question_number)
#Standardazing the data
mat_2<- heat_map_data_matrix_perceived[, c("22", "23", "24", "25")]
mat_2<- mat_2/max(mat_2)

#dput(heat_map_data_perceived$expert_in)

rownames(mat_2)<- c("Antioquia", "Caldas", "Cesar", "Chocó", "Córdoba", "Huila", 
"La Guajira", "Magdalena", "Quindío", "Risaralda", "Antioquia ", 
"Arauca", "Atlántico", "Bolívar", "Caldas ", "Casanare", "Cauca", 
"Córdoba ", "Huila ", "La Guajira ", "Magdalena ", "Meta", "Nariño", 
"Norte De Santander", "Quindío ", "Risaralda ", "Santander", "Sucre", 
...
#read the Sup_question_15_weighted
implementation<- read.csv("./Data/SUP_question_15_weighted.csv")
#count the number of expert_IDs by department in the data set
experts<- implementation %>%
  group_by(expert_in, crop) %>%
  summarise(total_experts= n())

# count the frequencies of the answer_in_english column of the implementation data set by expert_in and crop using the count_elements_by_group function
question_15_frequencies<- count_elements_by_group(implementation, "answer_in_english", c("crop", "expert_in"))

#Join the question_15_frequencies and experts data set 
question_15_frequencies<- inner_join(question_15_frequencies, experts)
# calculate the percentage dividing n to total_experts
question_15_frequencies$percentage<-question_15_frequencies$n/question_15_frequencies$total_experts
#reformat the wide format
question_15_frequencies<- dcast(data = question_15_frequencies, formula = crop+expert_in~answer_in_english)
#Standardazing the data
write.csv(question_15_frequencies, "./Data/SUP_Implentation_of_management_practices.csv")
mat_3<- question_15_frequencies[, c("Biological Control", "Biosecurity Measures", 
"Chemical Control", "Community Participation", "Cultural Control", 
"Education and Training", "Integrated Pest and Disease Management", 
"Monitoring and Reporting", "Physical Control", "Regulatory Compliance")]
...
implementation<- read.csv("./Data/SUP_Implentation_of_management_practices.csv")
#creating the matrix for the heatmap
mat_3<- implementation[, c("Integrated.Pest.and.Disease.Management", 
"Chemical.Control", "Community.Participation", "Monitoring.and.Reporting", 
"Cultural.Control", "Physical.Control", "Regulatory.Compliance", 
"Education.and.Training", "Biosecurity.Measures", "Biological.Control"
)]
#selecting the row names
rownames(mat_3)<- c("Antioquia", "Caldas", "Cesar", "Chocó", "Córdoba", "Huila", 
"La Guajira", "Magdalena", "Quindío", "Risaralda", "Antioquia ", 
"Arauca", "Atlántico", "Bolívar", "Caldas ", "Casanare", "Cauca", 
"Córdoba ", "Huila ", "La Guajira ", "Magdalena ", "Meta", "Nariño", 
"Norte De Santander", "Quindío ", "Risaralda ", "Santander", "Sucre", 
...
mat_final<- cbind(mat_3, mat, mat_2)
#Colnames
# colnames(mat_final)<- c("Int. Pest. Mang.", "Chem. Contr.", 
# "Community.Parti.", "Monit. and Report.", "Cult. Contr.", 
# "Phys. Contr.", "Regu. Compliance", "Edu. and Trai.", 
# "Biosec. Meas.", "Bio. Contr.", "Chem. Contr.", "Bio. Contr.", "Cult. Contr.", 
# "Biosec. Meas.", "Chem. Contr.", "Bio. Contr.", "Cult. Contr.", "Biosec. Meas.")


colnames(mat_final) <- c("Integrated Pest Management", "Chemical Control", 
                         "Community Participation", "Monitoring and Reporting", "Cultural Control", 
                         "Physical Control", "Regulatory Compliance", "Education and Training", 
                         "Biosecurity Measures", "Biological Control", "Chemical Control", "Biological Control", "Cultural Control", 
                         "Biosecurity Measures", "Chemical Control", "Biological Control", "Cultural Control", "Biosecurity Measures")


# 
# 
# 
# column_ha = HeatmapAnnotation(Level=c(rep("Implementation", ncol(mat_3)), rep("Correct utilization", ncol(mat)), rep("Perceived effectiveness", ncol(mat_2))))
# 
# 
# row_ha = rowAnnotation(crop = heat_map_data_matrix$crop)
# 
# 
# 
# Heatmap(mat_final, name = "mat_final", 
...
# Assuming mat_final is your matrix
# column_ha is the column annotation which you may need to define
# row_ha is the row annotation which you may need to define

# 1. Color setting for the heatmap
my_colors <- colorRamp2(c(min(mat_final, na.rm = TRUE), 0, max(mat_final, na.rm = TRUE)), 
                        c("#FFF2CC", "white", "#654321"))

# 2. Color setting for 'crop' annotation
row_ha = rowAnnotation(crop = heat_map_data_matrix_correct$crop, col = list(crop = c("Banana" = "#FFDA00", "Plantain" = "#28B463")))


# 3. Reorder the levels for the 'Level' annotation
# You need to create your column_ha with the levels in the correct order
# This is a placeholder for the actual code you would use to create your column_ha
column_ha_data<- c(rep("Implementation", ncol(mat_3)), rep("Correct utilization", ncol(mat)), rep("Perceived effectiveness", ncol(mat_2)))
column_ha_levels_ordered <- HeatmapAnnotation(Level = factor(column_ha_data, levels = c("Implementation", "Correct utilization", "Perceived effectiveness")),col = list(Level = c("Implementation" = "#654321", "Perceived effectiveness" = "#9E825C", "Correct utilization" = "#D8C298")))

# Replace all NA values with 0
mat_final[is.na(mat_final)] <- 0

# Create the Heatmap
Heatmap(mat_final, name = "Standard scale",
        col = my_colors,
        top_annotation = column_ha_levels_ordered,
        right_annotation = row_ha,
        cluster_columns = FALSE,
        na_col = "white")
...
# Extracting the "Biosec. Meas." column. Ensure the column name matches exactly.
biosec_meas_matrix <- mat_final[, "Biosecurity Measures", drop = FALSE]  # drop=FALSE to keep it as a matrix

# Assuming you want to keep the row annotations as is but remove column annotations since it's only one column.
# If you still want to show that this is the "Biosec. Meas." you can modify this according to your needs.

# Color setting for the heatmap
my_colors <- colorRamp2(c(min(biosec_meas_matrix, na.rm = TRUE), 0, max(biosec_meas_matrix, na.rm = TRUE)), 
                        c("#FFF2CC", "white", "#654321"))

# # Assuming you want to keep row annotations related to 'crop'
# # Ensure heat_map_data_matrix$crop exists or adjust as necessary to your data structure
# row_ha = rowAnnotation(crop = heat_map_data_matrix$crop, col = list(crop = c("Banana" = "#FFDA00", "Plantain" = "#28B463")))

# Create the Heatmap for "Biosec. Meas." column
Heatmap(biosec_meas_matrix, name = "Biosecurity Measures",
        col = my_colors,
        right_annotation = row_ha,
        cluster_columns = FALSE,
        cluster_rows = FALSE
        )  # Adjust clustering as per your analysis needs

# Note: The clustering, color annotations, and other features should be adjusted based on your specific analysis goals and the data characteristics.
