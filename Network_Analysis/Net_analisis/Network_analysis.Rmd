...
I have two datasets: DATA_MAP_Net_BC.csv (referred to as BC) and DATA_MAP_Net_BN.csv (referred to as BN), both of which are adjacency binary matrices. These matrices have identical dimensions, with corresponding rows and columns.

I need to identify the cells where the value is 0 in BC but non-zero in BN, and vice versa—where the value is 0 in BN but non-zero in BC.

For cells that are 0 in BC but non-zero in BN, I will assign a value of 100 in a new BN dataset. Conversely, for cells that are 0 in BN but non-zero in BC, I will assign a value of 200 in a new BC dataset.
...
BC <- fread("DATA_MAP_Net_BC.csv")
BN <- fread("DATA_MAP_Net_BN.csv")

# Create copies of BC and BN to store the new values
new_BC <- BC
new_BN <- BN

# Identify cells where BC is 0 and BN is non-zero, then set those cells to 100 in the new BN dataset
new_BN[BC == 0 & BN != 0] <- 100

# Identify cells where BN is 0 and BC is non-zero, then set those cells to 200 in the new BC dataset
new_BC[BN == 0 & BC != 0] <- 200

# Write the new datasets to CSV files
fwrite(new_BN, "New_DATA_MAP_Net_BN.csv")
fwrite(new_BC, "New_DATA_MAP_Net_BC.csv")
...
This relative exchange proportions will be multiplied by the matrix of qualitative exchange of planting material `DATA_MAP_Net_BC.csv`, `DATA_MAP_Net_BN.csv`,`DATA_MAP_Net_PN.csv`,`DATA_MAP_Net_PC.csv` to get the relative link weights of the network of planting material. ***It is important to notice that rare events are reported in the qualitative exchange of planting material (obtained from the discussion section of the EKE), but were not relatively reported in the individual surveys. Therefore, the relative exchange proportions of these events are set as 0.005 and the proportions were normalize to sum up 1***

# Multiply the matrixes and correcto for rare events!!!!
...
# 
# #Multiply the relative exchange proportions by the matrix of qualitative exchange of planting material for each category
# #Banana
# rel_prop_banana_form<- rel_prop_banana*binary_matrices[["BC"]]
# rel_prop_banana_inf<- rel_prop_banana*binary_matrices[["BN"]]
# #Plantain
# rel_prop_plantain_form<- rel_prop_plantain*binary_matrices[["PC"]]
# rel_prop_plantain_inf<- rel_prop_plantain*binary_matrices[["PN"]]
# 
